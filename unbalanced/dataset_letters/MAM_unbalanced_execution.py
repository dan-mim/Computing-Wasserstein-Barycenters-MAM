...
with open('data_letters_MAM_upper.pkl',
          'rb') as f:  # data_letters_mAM_lower data_letters_MAM_upper
    b = pickle.load(f)
# b = b[3][:60]
######################## pkl ad b[:50] !!!!!!!!!!!! ##########
...
l_gamma = [10**-3] #, 1, 10, 100] # [1000, 300, 200, 100, 70, 50, 10, 1, 10**-3] # [10**-4, 10**-3, 10**-2, 10**-1, 1, 10] #, 0]
# l_rho = [50]
for gamma in l_gamma:
    ...
    if rank == 0:
        with open(f'local_MAM_{pool_size}parallel_{TIME_SPENT}s_M_{len(b)}_{rho}rho_{gamma}unbalanced_letters_MAM_upper.pkl', 'wb') as f:
