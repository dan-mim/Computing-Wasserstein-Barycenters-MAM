...
# Function that project vector onto the subsimplex (the interior simplex using <= while the simplex is only =)
def projection_simplex_inf(V, z=1, eta=0, axis=None):
    """
    Projection of V inside the simplex, scaled by z:
    P(x; z) = argmin_{y >= 0, sum(y) <= z} ||y - V||^2
    z: float or array
        If array, len(z) must be compatible with V
    axis: None or int
    axis=None: project V by P(V.ravel(); z)
    ...
    axis=0: project each V[:, j] by P(V[:, j]; z[j])
    """
    # # unbalanced case with only unbalancy on the barycenter (the offer)
    # if eta == 0:
    #     return(projection_simplex(V, z=z, axis=axis))

    # unbalanced case with unbalancy on both the demand and the offer:
    V1 = V.clip(min=0)
    # V is inside the simplex:
    if np.sum(V1) <= z:
        return(V1)
    # V is not inside the simplex:
    return(projection_simplex(V, z=z, axis=axis))


# Optimized Operator Splitting method
def MAM_double_general(b, M_dist, computation_time=500, iterations_min=100, iterations_max=100000, rho=1000,
                                  nb_pixel_side=40, gamma=0, eta=0):
    """
    Input:
    ...
    *gamma: (float) unbalanced parameter concerning the barycenter -> if gamma != 0 then the algorithm goes directly in the unbalanced configuration
    *eta: (float) unbalanced parameter concerning the measures -> if eta != 0 then the algorithm goes directly in the unbalanced configuration
    ...
    *time_spent: (float) total time spent
    *Pi: M matrices of transport 
    *iterations_k: (int) number of iterations 
    ...
    # Take into account the unbalancy on the measures
    M_dist = M_dist - eta
    ...
    for m in range(M_):
        ...
        theta[m] = 10**-6 *( -1 / rho * M_dist[:, I] + np.ones(n_not_null) / R )
        PI[m] = -1 / rho * M_dist[:, I] + np.ones(n_not_null) / R
    ...
    while (iterations_k < iterations_min) or (spent_time < computation_time and iterations_k < iterations_max):
        ...
        for m in splitting_work[rank]:
            ...
            PI[m] = projection_simplex_inf(theta[m], z=1, eta=eta, axis=0) * b[m][I]
            theta[m] = PI[m] + t_unbalanced * np.expand_dims(deltaU, axis=1)
    ...
    for m in splitting_work[rank]:
        pi[m] = PI[m]
    Pi_gather = comm.gather(pi, root=0)
...
#     sys.stdout.flush()
    ...
    # Output
    return (p, Time, time_spent, Pi_gather,  iterations_k) #Pi # P,
