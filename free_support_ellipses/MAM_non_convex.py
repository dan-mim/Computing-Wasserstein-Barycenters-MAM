...
def MAM_large_sparse_support(b, M_dist=False, exact=False, keep_track=True, computation_time=100,
                             iterations_min=3, iterations_max=1000, precision=10**-4, rho=1000, evry_it = 10, name='MAM.pkl', visualize=False):
    """
    ...
    *exact: (bool) if True we use the free support method (large support considered, refer to Borckwardt), else fixed support, 
    meaning we keep the same support than have the b[i]s
    ...
    *iterations_k: (int) number of iterations 
    ...
    while (iterations_k < iterations_min) or (spent_time < computation_time and iterations_k < iterations_max and evol_p>precision): # and count_stop<10):  #  # and evol_p>10**-6) : # and evol_p>10**-16
        ...
        for m in splitting_work[rank]:
            ...
            theta[m] = theta[m] - 1 / rho * Mat_dist[m] + 2 * deltaU
    ...
    # Output
    return (p, P, Time, Wdist, l_precisionB, Precision, Iterations, total_time, iterations_k)  #Pi_tot , P,
